...
export default function Navbar({ onToggleSidebar }){
    ...
    async function handlesearch(e) {
        ...
        //0b73711396bc4420aaa164136aa648e9
        try {
            const response = await fetch(`https://api.rawg.io/api/games?key=key&search=${query}`);
        ...
        <nav className="bg-none w-screen flex px-4 py-3 justify-between z-40">
            ...
            <div className="relative">
                ...
                <div className="hidden peer-focus:block absolute left-0 right-0 bg-gray-800 w-fit max-h-screen overflow-y-auto z-50 mt-2 mx-auto">
                    {games.map( game => {
                        return(
                            <Link to={`/game/${game.id}`} key={game.id} className=" p-2 rounded hover:bg-gray-700 flex flex-col items-center">
